...
![策略績效圖](./equity_curve/Cumulative_Equity0515.png)
> **註**：回測區間涵蓋 500 天之市場多空循環，使用 300 檔高流動性標的進行跨截面模擬。
...
我們以兩個核心維度定義市場狀態：
- **價格波動度（Volatility）**：衡量市場情緒的劇烈程度
- **價格擴張程度（Price Expansion/Contraction）**：衡量價格相對於近期基準的偏離方向與力道

由此將市場劃分為四個象限，並根據象限轉移訊號動態調整持倉：
- 在**低估築底區（低波動+價格擴張）** 逐步布局
- 在**趨勢擴張區（中低波動+價格擴張）** 跟隨多頭動能
- 在**過熱或恐慌區** 適時降低曝險或退出
...
本專案實作了一套針對 **台股前 300 大高流動性資產** 的量化交易模型。核心邏輯透過「波動 」與「價格擴張 」兩大維度，將市場動態切分為四個象限，並動態捕捉象限間的轉移訊號進行資產配置。與傳統單一因子模型不同，本策略專注於在不同市場環境下進行自適應的權重調整，旨在維持極低的組合波動度。在回測中展現了顯著的風險調整後收益（Sharpe Ratio 2.056），具備高度的穩定性。
...
總報酬率35.08%，顯示在726天內（含225天暖機期）資產增值了約 35.08%。將總報酬轉換為每年的複利成長率，年化報酬率約為 24.49%，反映了策略的長期獲利能力。年化波動率10.95%，顯示資金曲線非常平穩，沒有大幅度的上下震盪。

衡量每單位總風險所能換取之超額報酬，Sharpe Ratio 約為2.056，顯示風險與報酬相對平衡。針對下行風險（虧損）之評估，Sortino Ratio (2.87) 比Sharpe Ratio高，顯示此策略在避開虧損方面的表現優於對應整體波動的表現。年化報酬率除以最大回撤Calmar Ratio (2.36)，反映了策略在獲利與最大潛在虧損之間的性價比。數值大於 1，表現尚可。

在高風險的情況下，最大回撤為10.38%，顯示資產從最高點跌落到最低點的最大幅度，表現出此策略穩健的特性。惟最大回撤持續時間接近一年（約 299 天），資產從回落到創新高耗時良久，考驗投資人耐心。

從統計分佈來看，因為偏度 (-0.74)為負，代表報酬分布呈現「左偏」，即平時小賺多，但偶爾會出現較大的單日虧損。超額峰度 (2.404)屬於「肥尾」分布，代表極端事件（暴漲或暴跌）發生的機率高於常態分布。然而，尾部比率 (0.86)接近1，代表兩端風險相對對稱。VaR (-0.01%)顯示在95% 的信心水準，單日預計的最大虧損不會超過 0.01%。
...
### 1. 隨機訊號回測 (Monte Carlo Benchmark)
