...
### II. Parameter Optimization & Sensitivity Analysis
- **Optimization Method**: Grid Search was applied for parameter scanning across decision thresholds.
- **Optimization Results**: The test results displayed a distinct "Parameter Plateau" characteristic. When the decision threshold was fine-tuned within the target range (e.g., ±10%), both the Sharpe Ratio and profitability remained stable, with no sharp performance drop-offs (Cliff Effect). This confirms that the strategy possesses high robustness and tolerance regarding threshold settings.
...
1. **Time Path Dependency and Sensitivity to Time Series**: Although the backtest covers approximately 10 years of data, Stationary Bootstrap results show that strategy performance is somewhat sensitive to the "time series path," indicating that the timing advantage of entry and exit is not absolute. If the temporal structure of future market returns shifts, actual performance may diverge from backtest results.
2. **Risk of Cross-Sectional Alpha Decay Due to Structural Market Changes**: Cross-section Permutation tests support that the strategy possesses significant stock-selection and exclusion capabilities. However, this advantage is built upon the current market's sector rotation, stock correlation, and risk premium structure. Should long-term style rotations occur (e.g., factor decay, liquidity structural shifts, or regulatory changes), cross-sectional alpha may weaken.
3. **Parameter Search and Data-Snooping Risks**: Strategy development involves parameter optimization. Although significance testing was conducted using Permutation and Bootstrap methods, data-snooping risk cannot be entirely eliminated. In practice, frequent re-optimization based on the latest data should be avoided.
4. **Considerations for Omitting Walk Forward Analysis**:
    - **Hypothesis-Driven Logic**: The core scoring mechanism of this strategy is based on market structure and empirical hypotheses (Hypothesis-Driven), rather than pure data mining.
    - **Cross-Period Stability Testing**: After optimizing decision thresholds using recent data, reverse stress testing was conducted directly on earlier historical market data. Results indicate that the same thresholds retain strong discriminative power in earlier market conditions.
    - **Avoiding Overfitting to Noise**: Given that parameters exhibit a significant "parameter plateau" and consistent performance across periods, maintaining fixed thresholds avoids the risk of chasing short-term market noise caused by frequent Walk Forward re-optimization.
5. **Limited Out-of-Sample (OOS) Validation Period**: Although multi-segment OOS Bootstrap results show significant positive expectation, the out-of-sample testing period remains relatively limited, which cannot completely rule out random luck driven by specific market regimes (such as specific trends or extreme volatility phases).
6. **Trading Costs, Slippage, and Liquidity Constraints**: Backtest configurations cannot fully capture market impact costs, slippage, and execution failure risks present in live trading. For mid-to-low liquidity assets or during periods of market stress, actual execution costs may exceed assumptions, eroding net returns.
7. **Model Risk and Black Swan Event Defense**: The strategy relies on historical statistical patterns and offers limited defense against extreme black swan events (e.g., flash liquidity freezes or systemic financial crises). Statistical simulations cannot account for unobserved future risk factors.
8. **Limitations of Benchmarks and Comparisons**: While the strategy outperforms random holding and simple baselines, it does not guarantee long-term, consistent outperformance over broad market indices. Evaluation should be conducted in the context of overall asset allocation and individual risk tolerance, rather than relying solely on historical Sharpe ratios or equity curves.
-----
